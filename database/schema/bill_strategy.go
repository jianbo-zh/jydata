...
func (BillingStrategy) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int("main_mode").Comment("主要模式（0-按时间 1-按里程）"),
		field.Int("start_time_price").Comment("起步价(按时)收费价格(单位：分)"),
		field.Int("start_time_unit").Default(1).Comment("起步价(按时)计量单位（单位：秒）"),
		field.Int("normal_time_price").Comment("时间收费价格(单位：分)"),
		field.Int("normal_time_unit").Default(1).Comment("时间计量单位（单位：秒）"),
		field.Int("start_mileage_price").Comment("起步价(按里程)收费价格(单位：分)"),
		field.Int("start_mileage_unit").Default(1).Comment("起步价(按里程)计量单位（单位：米）"),
		field.Int("normal_mileage_price").Comment("里程收费价格(单位：分)"),
		field.Int("normal_mileage_unit").Default(1).Comment("里程计量单位（单位：米）"),
		field.Int("capped_amount").Comment("封顶价格（单位：分）"),
		field.Int("deposit_amount").Comment("押金金额（单位：分）"),
