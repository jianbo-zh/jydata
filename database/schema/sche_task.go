...
func (ScheTask) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("start_lon").Default(0).Comment("起点经度(wgs84)"),
		field.Float("start_lat").Default(0).Comment("起点纬度(wgs84)"),
		field.Float("dest_lon").Comment("目的地经度(wgs84)"),
		field.Float("dest_lat").Comment("目的地纬度(wgs84)"),
