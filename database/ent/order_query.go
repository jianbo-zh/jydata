...
type OrderQuery struct {
	...
	withBilling              *OrderBillingQuery
	withRefund               *OrderRefundQuery
	withSharing              *OrderSharingQuery
...
func (oq *OrderQuery) QueryCar() *CarQuery {
	query := (&CarClient{config: oq.config}).Query()
...
func (oq *OrderQuery) QueryBackgroundScenicArea() *ScenicAreaQuery {
	query := (&ScenicAreaClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrderQuery) QueryBilling() *OrderBillingQuery {
	query := (&OrderBillingClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QuerySharing chains the current query on the "sharing" edge.
func (oq *OrderQuery) QuerySharing() *OrderSharingQuery {
	query := (&OrderSharingClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withCar:                  oq.withCar.Clone(),
		withBackgroundScenicArea: oq.withBackgroundScenicArea.Clone(),
		withBilling:              oq.withBilling.Clone(),
		withRefund:               oq.withRefund.Clone(),
		withSharing:              oq.withSharing.Clone(),
...
func (oq *OrderQuery) WithCar(opts ...func(*CarQuery)) *OrderQuery {
	query := (&CarClient{config: oq.config}).Query()
...
func (oq *OrderQuery) WithBackgroundScenicArea(opts ...func(*ScenicAreaQuery)) *OrderQuery {
	query := (&ScenicAreaClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withBackgroundScenicArea = query
	return oq
}
...
func (oq *OrderQuery) WithBilling(opts ...func(*OrderBillingQuery)) *OrderQuery {
	query := (&OrderBillingClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withBilling = query
	return oq
}
...
// WithSharing tells the query-builder to eager-load the nodes that are connected to
// the "sharing" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithSharing(opts ...func(*OrderSharingQuery)) *OrderQuery {
	query := (&OrderSharingClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withSharing = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	var (
		...
		loadedTypes = [5]bool{
			oq.withCar != nil,
			oq.withBackgroundScenicArea != nil,
			oq.withBilling != nil,
			oq.withRefund != nil,
			oq.withSharing != nil,
		}
	...
	if query := oq.withCar; query != nil {
		if err := oq.loadCar(ctx, query, nodes, nil,
			func(n *Order, e *Car) { n.Edges.Car = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withBackgroundScenicArea; query != nil {
		if err := oq.loadBackgroundScenicArea(ctx, query, nodes, nil,
			func(n *Order, e *ScenicArea) { n.Edges.BackgroundScenicArea = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withBilling; query != nil {
		if err := oq.loadBilling(ctx, query, nodes, nil,
			func(n *Order, e *OrderBilling) { n.Edges.Billing = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withRefund; query != nil {
		if err := oq.loadRefund(ctx, query, nodes,
			func(n *Order) { n.Edges.Refund = []*OrderRefund{} },
			func(n *Order, e *OrderRefund) { n.Edges.Refund = append(n.Edges.Refund, e) }); err != nil {
			return nil, err
		}
	}
	if query := oq.withSharing; query != nil {
		if err := oq.loadSharing(ctx, query, nodes, nil,
			func(n *Order, e *OrderSharing) { n.Edges.Sharing = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadCar(ctx context.Context, query *CarQuery, nodes []*Order, init func(*Order), assign func(*Order, *Car)) error {
...
func (oq *OrderQuery) loadBackgroundScenicArea(ctx context.Context, query *ScenicAreaQuery, nodes []*Order, init func(*Order), assign func(*Order, *ScenicArea)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Order)
	for i := range nodes {
...
func (oq *OrderQuery) loadBilling(ctx context.Context, query *OrderBillingQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderBilling)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(orderbilling.FieldOrderID)
	}
	query.Where(predicate.OrderBilling(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.BillingColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (oq *OrderQuery) loadSharing(ctx context.Context, query *OrderSharingQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderSharing)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ordersharing.FieldOrderID)
	}
	query.Where(predicate.OrderSharing(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.SharingColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (oq *OrderQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := oq.ctx.Fields; len(fields) > 0 {
		...
		if oq.withCar != nil {
			_spec.Node.AddColumnOnce(order.FieldCarID)
		}
		if oq.withBackgroundScenicArea != nil {
			_spec.Node.AddColumnOnce(order.FieldScenicAreaID)
		}
	}
