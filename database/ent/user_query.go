...
func (uq *UserQuery) QueryBackgroundScenicArea() *ScenicAreaQuery {
	query := (&ScenicAreaClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withBackgroundScenicArea: uq.withBackgroundScenicArea.Clone(),
...
func (uq *UserQuery) WithBackgroundScenicArea(opts ...func(*ScenicAreaQuery)) *UserQuery {
	query := (&ScenicAreaClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withBackgroundScenicArea = query
	return uq
}
...
//
//	client.User.Query().
//		GroupBy(user.FieldScenicAreaID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [1]bool{
			uq.withBackgroundScenicArea != nil,
		}
	)
	...
	if query := uq.withBackgroundScenicArea; query != nil {
		if err := uq.loadBackgroundScenicArea(ctx, query, nodes, nil,
			func(n *User, e *ScenicArea) { n.Edges.BackgroundScenicArea = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadBackgroundScenicArea(ctx context.Context, query *ScenicAreaQuery, nodes []*User, init func(*User), assign func(*User, *ScenicArea)) error {
...
func (uq *UserQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := uq.ctx.Fields; len(fields) > 0 {
		...
		if uq.withBackgroundScenicArea != nil {
			_spec.Node.AddColumnOnce(user.FieldScenicAreaID)
		}
	}
