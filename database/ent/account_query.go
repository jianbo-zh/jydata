...
func (aq *AccountQuery) QueryBackgroundScenicArea() *ScenicAreaQuery {
	query := (&ScenicAreaClient{config: aq.config}).Query()
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withBackgroundScenicArea: aq.withBackgroundScenicArea.Clone(),
...
func (aq *AccountQuery) WithBackgroundScenicArea(opts ...func(*ScenicAreaQuery)) *AccountQuery {
	query := (&ScenicAreaClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withBackgroundScenicArea = query
	return aq
}
...
//		GroupBy(account.FieldScenicAreaID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Account.Query().
//		Select(account.FieldScenicAreaID).
//		Scan(ctx, &v)
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	var (
		...
		_spec       = aq.querySpec()
		loadedTypes = [1]bool{
			aq.withBackgroundScenicArea != nil,
		}
	)
	...
	if query := aq.withBackgroundScenicArea; query != nil {
		if err := aq.loadBackgroundScenicArea(ctx, query, nodes, nil,
			func(n *Account, e *ScenicArea) { n.Edges.BackgroundScenicArea = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadBackgroundScenicArea(ctx context.Context, query *ScenicAreaQuery, nodes []*Account, init func(*Account), assign func(*Account, *ScenicArea)) error {
...
func (aq *AccountQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withBackgroundScenicArea != nil {
			_spec.Node.AddColumnOnce(account.FieldScenicAreaID)
		}
	}
